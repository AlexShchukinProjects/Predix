...
function showPositionModal(flight, crew, onAssign) {
    ...
    fetch(`/api/aircraft-type-by-regn?regn=${encodeURIComponent(regn)}`)
        ...
        .then(typeData => {
            ...
            // 3. Получаем минимальный экипаж для типа ВС и данные о назначенных сотрудниках параллельно
            const flightId = flight.id || flight.ID || null;
            const minCrewPromise = fetch(`/api/minimum-crew/by-aircraft-type?aircraft_type_id=${aircraftTypeId}`)
                .then(response => response.json());
            
            // Загружаем данные о назначенных сотрудниках, если есть ID рейса
            const assignedCrewPromise = flightId 
                ? fetch(`/planning/flights/get-crew/${flightId}`)
                    .then(response => response.json())
                    .then(data => {
                        // Создаем мапу назначенных сотрудников по позициям
                        const assignedMap = {};
                        if (data.success && Array.isArray(data.crew_members)) {
                            data.crew_members.forEach(member => {
                                // Используем position_with_slot для маппинга (это позиция с номером слота типа "F/O1", "F/O2")
                                const key = member.position_with_slot || member.position;
                                assignedMap[key] = member.name;
                            });
                        }
                        return assignedMap;
                    })
                    .catch(error => {
                        console.warn('Failed to load assigned crew:', error);
                        return {};
                    })
                : Promise.resolve({});

            return Promise.all([minCrewPromise, assignedCrewPromise]);
        })
        .then(([minCrewData, assignedCrewMap]) => {
            ...
            return fetch('/api/positions')
                ...
                .then(posData => {
                        ...
                        groupItems.forEach((item) => {
                            ...
                            if (quantity > 0) {
                                ...
                                for (let i = 1; i <= quantity; i++) {
                                    ...
                                    // Всегда добавляем номер слота для консистентности с API
                                    const positionValue = `${positionShortName}${i}`;
                                    
                                    // Проверяем, назначен ли кто-то на эту позицию
                                    const assignedName = assignedCrewMap[positionValue] || null;
                                    ...
                                    // Добавляем ФИО назначенного сотрудника справа, если он есть
                                    if (assignedName) {
                    ...
                    document.getElementById('position-ok').onclick = function() {
                        ...
                        console.log('=== REQUIREMENTS CHECK START (public/js) ===');
                        console.log('Crew object in RectMovement (public/js):', crew);
                        console.log('aircraftTypeIcaoForChecks in RectMovement (public/js):', aircraftTypeIcaoForChecks);

                        try {
                            // Глобальная карта статусов сотрудников формируется в crew-schedule-chart.js и экспортируется как window.crewStatusMap
                            const globalCrewStatusMap = (typeof window !== 'undefined' && window.crewStatusMap) ? window.crewStatusMap : null;
                            console.log('window.crewStatusMap exists (public/js):', !!globalCrewStatusMap);

                            // Определяем ID сотрудника (в MemberJson он называется id, см. crew-schedule-chart.js)
                            const crewIdForStatus = crew && (crew.id || crew.ID || crew.crew_id || crew.person_id || null);
                            console.log('Resolved crewIdForStatus (public/js):', crewIdForStatus);

                            if (globalCrewStatusMap && crewIdForStatus) {
                                const crewStatus = globalCrewStatusMap[crewIdForStatus];
                                console.log('Found crewStatus for requirements check (public/js):', crewStatus);
                                ...
                                if (crewStatus && Array.isArray(crewStatus.aircraft_types) && crewStatus.aircraft_types.length > 0) {
                                    ...
                                    if (aircraftTypeIcaoForChecks) {
                                        ...
                                        console.log('Matched typeStatus by ICAO (public/js):', typeStatus);
                                    }

                                    // Если точное сопоставление не найдено, берем первый доступный тип как фолбэк
                                    if (!typeStatus) {
                                        console.warn('Type-specific status for aircraft not found, using first available aircraft_type entry for crew (public/js)');
                                        typeStatus = crewStatus.aircraft_types[0];
                                    }

                                    if (typeStatus && Array.isArray(typeStatus.requirements)) {
                                        console.log('typeStatus for requirements check (public/js):', typeStatus);
                                        ...
                                        console.log('problemRequirements for this assignment (public/js):', problemRequirements);
                                        ...
                                        if (problemRequirements.length > 0) {
                                            ...
                                            showRequirementsWarningModal(
                                                ...
                                                function() {
                                                    ...
                                                    modal.remove();
                                                    console.log('Calling onAssign with position after requirements warning (public/js):', pos);
                                                    onAssign(pos);
                                                }
                                            );
                                        ...
                                        console.log('typeStatus is missing or has no requirements array (public/js)');
                                    }
                                }
                            } else {
                                console.log('No globalCrewStatusMap or crewIdForStatus not resolved; skipping requirements warning (public/js)');
                            }
                        } catch (err) {
                            console.error('Error while checking crew requirements before assignment (public/js):', err);
                        ...
                        modal.remove();
                        console.log('Calling onAssign with position (no blocking requirements, public/js):', pos);
                        onAssign(pos);
                    };
...
function showRequirementsWarningModal(crew, aircraftTypeIcao, requirements, onContinue) {
    console.log('showRequirementsWarningModal called with (public/js):', { crew, aircraftTypeIcao, requirements });
    ...
    try {
        ...
        console.error('Error while building requirements list for warning modal (public/js):', e);
