...
<div class="container mt-4">
    ...
    <form action="{{ route('events.update', $event) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="abbreviation" name="abbreviation" value="{{ old('abbreviation', $event->abbreviation) }}" maxlength="10" placeholder="Например: ТР, ПК, ОБ">
            @error('abbreviation')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <div class="d-flex align-items-center">
                <input type="color" class="form-control form-control-color me-2" id="color" name="color" value="{{ old('color', $event->color ?? '#007bff') }}" style="width: 50px; height: 38px;">
                <input type="text" class="form-control" id="colorHex" placeholder="#007bff" value="{{ old('color', $event->color ?? '#007bff') }}" pattern="^#[0-9A-Fa-f]{6}$" maxlength="7">
            </div>
        ...
        <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="work_type" name="work_type" value="1" {{ $event->work_type ? 'checked' : '' }}>
            <label class="form-check-label" for="work_type">Рабочее</label>
        </div>
        <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="active" name="active" value="1" {{ $event->active ? 'checked' : '' }}>
