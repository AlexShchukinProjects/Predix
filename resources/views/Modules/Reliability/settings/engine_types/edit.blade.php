...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h4 mb-0">
            {{ isset($item) ? 'Редактировать тип двигателя' : 'Добавить тип двигателя' }}
        </h1>
    </div>
        ...
        <form method="POST" action="{{ isset($item)
            ...
            <div class="card-footer d-flex justify-content-between align-items-center">
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('modules.reliability.settings.engine-types.index') }}" class="btn btn-outline-primary">Отмена</a>
                </div>

                @if(isset($item))
                    <form method="POST" action="{{ route('modules.reliability.settings.engine-types.destroy', $item) }}" onsubmit="return confirm('Удалить запись?');">
                        @csrf
                        @method('DELETE')
