...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h4 mb-0">
            {{ isset($item) ? 'Edit WO status' : 'Add WO status' }}
        </h1>
    </div>
        ...
        <form method="POST" action="{{ isset($item)
            ...
            <div class="card-footer d-flex justify-content-between align-items-center">
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('modules.reliability.settings.wo-statuses.index') }}" class="btn btn-outline-primary">Cancel</a>
                </div>

                @if(isset($item))
                    <form method="POST" action="{{ route('modules.reliability.settings.wo-statuses.destroy', $item) }}" onsubmit="return confirm('Delete record?');">
                        @csrf
                        @method('DELETE')
