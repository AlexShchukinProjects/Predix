...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h4 mb-0">
            {{ isset($item) ? 'Редактировать этап обнаружения отказа' : 'Добавить этап обнаружения отказа' }}
        </h1>
    </div>
    ...
    <div class="card" style="max-width: 800px;">
        <form method="POST" action="{{ isset($item)
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    <label class="form-label">Родительский этап</label>
            ...
            <div class="card-footer d-flex justify-content-between align-items-center">
                <div class="efds-actions mb-0">
                    ...
                    <a href="{{ route('modules.reliability.settings.detection-stages.index') }}" class="btn efds-btn efds-btn--outline-primary">Отмена</a>
                </div>
            </div>
        </form>
        ...
        @if(isset($item))
            <div class="card-footer d-flex justify-content-end border-0 pt-0">
                <form method="POST" action="{{ route('modules.reliability.settings.detection-stages.destroy', $item) }}" onsubmit="return confirm('Удалить запись?');">
                    @csrf
                    @method('DELETE')
