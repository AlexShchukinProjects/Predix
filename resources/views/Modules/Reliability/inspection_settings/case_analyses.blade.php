...
<div class="container-fluid py-3">
    ...
    <div class="efds-table-header">
        <div class="efds-table-header__stats text-muted">
            ...
            <select class="form-select form-select-sm" id="case-analyses-per-page" aria-label="Записей на странице">
        ...
        <div class="efds-table-header__actions">
            <button type="button" class="btn efds-btn efds-btn--outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#caseAnalysesUploadModal"><i class="fas fa-file-excel me-1"></i>Добавить из Excel / CSV</button>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm mb-0">
                        <thead class="table-light">
                            <tr>
                                <th class="text-center" style="width: 2.5rem;"><input type="checkbox" id="case-analyses-select-all" class="form-check-input" title="Выбрать все на странице"></th>
                                <th>id</th>
                                <th>work_card_id</th>
                                <th>tc_number</th>
                                <th>file_path</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $row->file_path }}</td>
                                <td>{{ $row->file_name }}</td>
                                <td>{{ $row->is_critical ? 'Да' : 'Нет' }}</td>
                                <td>{{ $row->remarks }}</td>
                                <td>{{ $row->created_at?->format('Y-m-d H:i') }}</td>
                                <td>{{ $row->updated_at?->format('Y-m-d H:i') }}</td>
                            </tr>
                            @empty
                            <tr><td colspan="10" class="text-muted">Нет данных. Загрузите CSV или XLSX.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="caseAnalysesUploadModalLabel">Добавить из Excel / CSV</h5>
            ...
            <form id="form-upload-case-analyses-modal" action="{{ route('modules.reliability.settings.inspection.case-analyses.upload') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-footer">
                    <div class="efds-actions mb-0">
                        ...
                        <button type="submit" id="case-analyses-upload-submit" class="btn efds-btn efds-btn--primary" disabled>Загрузить</button>
                    </div>
                </div>
            </form>
        </div>
