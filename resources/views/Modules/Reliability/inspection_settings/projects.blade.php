...
<div class="container-fluid py-3">
    ...
    <div class="efds-table-header">
        <div class="efds-table-header__stats text-muted">
            ...
            <select class="form-select form-select-sm" id="projects-per-page" aria-label="Записей на странице">
        ...
        <div class="efds-table-header__actions">
            <button type="button" class="btn efds-btn efds-btn--outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#projectsUploadModal"><i class="fas fa-file-excel me-1"></i>Добавить из Excel / CSV</button>
            ...
            <form id="form-delete-projects" action="{{ route('modules.reliability.settings.inspection.projects.delete') }}" method="post" class="d-none">
                @csrf
                <button type="submit" id="btn-delete-projects" class="btn efds-btn efds-btn--danger btn-sm">Удалить выбранные</button>
            </form>
        </div>
    </div>
                ...
                <div class="reliability-table-scroll-wrap">
                    <div class="table-responsive">
                        ...
                        <thead class="table-light">
                            <tr>
                                <th class="text-center" style="width: 2.5rem;"><input type="checkbox" id="projects-select-all" class="form-check-input" title="Выбрать все на странице"></th>
                                <th>id</th>
                                <th>PROJECT #</th>
                                <th>STATUS</th>
                                <th>TAIL NUMBER</th>
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="68" class="text-muted">Нет данных. Загрузите CSV или XLSX.</td></tr>
                            @endforelse
                        </tbody>
                        </table>
                    </div>
...
{{-- Модальное окно: загрузка из Excel/CSV с перетаскиванием --}}
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="projectsUploadModalLabel">Добавить из Excel / CSV</h5>
            ...
            <form id="form-upload-projects-modal" action="{{ route('modules.reliability.settings.inspection.projects.upload') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-footer">
                    <div class="efds-actions mb-0">
                        ...
                        <button type="submit" id="projects-upload-submit" class="btn efds-btn efds-btn--primary" disabled>Загрузить</button>
                    </div>
                </div>
            </form>
        </div>
...
/* Растягиваем main_screen на всю ширину — как на /modules/reliability */
.main_screen {
    width: 100% !important;
}
/* Контейнер страницы занимает 100% и не добавляет горизонтальный скролл */
.projects-table-card {
    width: 100%;
    overflow: hidden;
}
...
/* Фиксированная первая строка (заголовок) таблицы при вертикальной прокрутке */
...
/* Отступ между таблицей и пагинацией — пагинация не залезает на скроллбар */
.efds-pagination-wrap {
    margin-top: 1rem !important;
    padding-top: 0.75rem;
}
/* Блок «На странице» / «Всего записей» — без переноса текста */
...
/* Зона перетаскивания в модальном окне */
...
(function() {
    ...
    // Модальное окно загрузки: перетаскивание и выбор файла
