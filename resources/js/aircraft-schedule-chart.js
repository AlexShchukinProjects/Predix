...
try {
    ...
    if (colorsNode && colorsNode.dataset && colorsNode.dataset.maps) {
        flightStatusColorsMap = JSON.parse(colorsNode.dataset.maps);
        console.log('📊 flightStatusColorsMap загружен:', flightStatusColorsMap);
    }
} catch (e) {
    console.error('Ошибка загрузки flightStatusColorsMap:', e);
}
...
//рисуем подписи дней и вертикальные линии
    ...
    for(let i1=0;i1<periodNew;i1++){
        ...
        for(let j=0;j<=numberHours;j++){
            ...
            ctx.font = "12px Arial";
            ctx.fillStyle="white";
            if (periodNew==7) HoursText=6; else HoursText=1;
...
// линия текущего времени
    ...
    // Линии между соседними рейсами с одинаковым номером маршрута (trip_number)
    drawRouteLines();
...
/**
 * Рисует линии между соседними по времени рейсами с одинаковым trip_number (один маршрут).
 * Рейсы могут быть в data в любом порядке — сортируем по x (времени) в каждой строке ВС.
 */
function drawRouteLines() {
    const rowTol = 3;
    const flightRects = rectHover.filter(function (r) {
        return r && r.activity_type === 'FL' && r.trip_number != null && r.trip_number !== '';
    });
    if (flightRects.length < 2) return;

    // Группируем по строке (y — одна и та же ВС)
    const byRow = {};
    flightRects.forEach(function (r) {
        const y = Math.round((r.y || 0) / rowTol) * rowTol;
        if (!byRow[y]) byRow[y] = [];
        byRow[y].push(r);
    });
    ...
    Object.keys(byRow).forEach(function (rowKey) {
        ...
        for (let i = 0; i < rects.length - 1; i++) {
            const r1 = rects[i];
            const r2 = rects[i + 1];
            if (r1.trip_number != r2.trip_number) continue;
            const x1 = (r1.x || 0) + (r1.XLong || 0);
            const y1 = (r1.y || 0) + ((r1.heightRectFlight || 25) / 2)+10;
            const x2 = r2.x || 0;
            const y2 = (r2.y || 0) + ((r2.heightRectFlight || 25) / 2)+10;
            ctx.beginPath();
            ctx.moveTo(x1, y1);
            ctx.lineTo(x2, y2);
            ctx.stroke();
...
//    console.log("y",y);
    // Находим текущий полет в data по уникальному идентификатору рейса
    // Это позволяет корректно различать несколько рейсов с одинаковым номером и бортом
    ...
    if (currentFlight && 
        ...
        const takenOffColor = resolveStatusColorFromDict('Взлетел');
        console.log('🛫 Рейс взлетел:', flight_number, 'actual_date_departure:', currentFlight.actual_date_departure, 'actual_time_departure:', currentFlight.actual_time_departure, 'actual_date_arrival:', currentFlight.actual_date_arrival, 'takenOffColor:', takenOffColor);
        if (takenOffColor) {
    ...
    if(activity_type === 'maintanence') {
        // Получаем цвет из базы данных, если его нет - используем дефолтный
        const maintenanceColor = resolveStatusColorFromDict('Тех обслуживание');
        statusColor = maintenanceColor; // Оранжевый по умолчанию
    }
    ...
    if(currentFlight && currentFlight.actual_date_departure && currentFlight.actual_time_departure && 
       currentFlight.actual_date_arrival && currentFlight.actual_time_arrival) {
        ...
        if(activity_type === 'FL') {
            // Парсим фактические дату и время
            const actualDepDate = new Date(currentFlight.actual_date_departure + ' ' + currentFlight.actual_time_departure);
            const actualArrDate = new Date(currentFlight.actual_date_arrival + ' ' + currentFlight.actual_time_arrival);
            
            // Вычисляем начало периода (дата из dateQ в 00:00:00)
            const periodStartDate = new Date(date1.value);
            periodStartDate.setHours(0, 0, 0, 0);
            
            // Вычисляем минуты от начала периода
            let actualXStart = (actualDepDate - periodStartDate) / (1000 * 60);
            let actualXFinish = (actualArrDate - periodStartDate) / (1000 * 60);
            
            // Если начало меньше 0, значит рейс начался до начала периода
            if(actualXStart < 0) actualXStart = 0;
           
            // Рисуем прямоугольник для фактического времени (цвет из базы данных)
            const actualTimeColor = resolveStatusColorFromDict('Фактическое время');
            ctx.fillStyle = actualTimeColor || "rgb(255,159,10)"; // Оранжевый по умолчанию, если цвет не найден в базе
    ...
    rectHover.push({
        ...
        trip_number: trip_number != null && trip_number !== '' ? trip_number : null,
...
*/
            ...
            if((dx>dot.x) && (dx<(dot.x+dot.XLong)) && (dy>dot.y) && (dy<dot.y+heightRectFlight)) {
                ...
                if (tooltip) {
                    ...
                    // Форматируем фактические времена взлета и посадки
                    let fact_takeoff_time = '-';
                    if (dot.actual_time_departure) {
                        fact_takeoff_time = (dot.actual_time_departure || '').substr(0, 5) || '-';
                    }
                    
                    let fact_landing_time = '-';
                    if (dot.actual_time_arrival) {
                        fact_landing_time = (dot.actual_time_arrival || '').substr(0, 5) || '-';
                    }
                    ...
                    if (time_start && time_finish) {
                        try {
                            ...
                            const [finishHours, finishMinutes] = time_finish.split(':').map(Number);
                            const startTotalMinutes = startHours * 60 + startMinutes;
                            const finishTotalMinutes = finishHours * 60 + finishMinutes;
                            let durationMinutes = finishTotalMinutes - startTotalMinutes;
                            
                            // Если время прилета меньше времени вылета, значит прилет на следующий день
                            if (durationMinutes < 0) {
                                durationMinutes += 24 * 60; // Добавляем сутки
                            }
                            
                            const hours = Math.floor(durationMinutes / 60);
                            const minutes = durationMinutes % 60;
                            flightDuration = `${hours}ч ${minutes}мин`;
                        } catch (e) {
                            flightDuration = '-';
                        }
                    }
                    ...
                    tooltip.innerHTML = `
                        ...
                        <div>Время вылета ФАКТ: ${fact_takeoff_time}</div>
                        <div>Время прилета: ${time_finishHM || '-'}</div>
                        <div>Время прилета ФАКТ: ${fact_landing_time}</div>
                        <div>Время полета: ${flightDuration}</div>
                        <div>Количество пассажиров: ${passengers_count || '-'}</div>
