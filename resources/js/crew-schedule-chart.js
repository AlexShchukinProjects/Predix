    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result.success) {
                ...
                // Делаем карту доступной глобально, чтобы другие скрипты (RectMovement.js) могли её использовать
                if (typeof window !== 'undefined') {
                    window.crewStatusMap = crewStatusMap;
                }
                console.log('Статусы сотрудников загружены:', crewStatusData);
                console.log('Глобальная карта window.crewStatusMap установлена:', typeof window !== 'undefined' ? window.crewStatusMap : undefined);
                return true;
            } else {
...
// Карта цветов статусов из скрытого div (статусы теперь хранятся по русским названиям)
...
// Функция для получения цвета статуса по названию (используется напрямую русское название из БД)
function resolveCrewStatusColor(statusName) {
    if (!flightStatusColorsMapCrew || !statusName) return null;
    // Прямой поиск по названию статуса
    if (flightStatusColorsMapCrew[statusName]) return flightStatusColorsMapCrew[statusName];
    // Поиск без учета регистра
    const lowerStatusName = statusName.toLowerCase();
    for (const key in flightStatusColorsMapCrew) {
        if (key.toLowerCase() === lowerStatusName) {
            return flightStatusColorsMapCrew[key];
        }
    }
    return null;
}
...
// линия текущего времени
    ...
    // Получаем выбранную дату начала периода
    const dateQInput = document.getElementById('dateQ');
    const startDate = dateQInput ? new Date(dateQInput.value + 'T00:00:00Z') : new Date();
    startDate.setUTCHours(0, 0, 0, 0);
    
    // Получаем текущую дату и время в UTC
    const now = new Date();
    const todayDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    const currentTimeOfDay = now.getUTCHours() * 60 + now.getUTCMinutes(); // минуты от начала дня в UTC
    
    // Вычисляем разницу в минутах между текущим временем и началом выбранного периода
    const startDateTimestamp = startDate.getTime();
    const currentDateTimeTimestamp = todayDate.getTime() + (currentTimeOfDay * 60 * 1000);
    const timeDiffMinutes = (currentDateTimeTimestamp - startDateTimestamp) / (60 * 1000);
    
    // Вычисляем позицию линии текущего времени
    const currentTimeX = longFirstColumn + timeDiffMinutes / Scale;
    const canvasEndX = canvas2.width;
    
    // Рисуем линию только если текущее время попадает в диапазон отображаемых дат
    if (currentTimeX >= longFirstColumn && currentTimeX <= canvasEndX && timeDiffMinutes >= 0 && timeDiffMinutes <= (periodNew * 24 * 60)) {
        ctx.beginPath();
    ...
    for (let position in allCrewPositions) {
        if (allCrewPositions.hasOwnProperty(position) && allCrewPositions[position]) {
            ...
            if(flight_active===true) {
                ...
                // Если цвет не передан, используем цвет по умолчанию
                if (!statusColor) {
                    statusColor = '#E8F5E9'; // светло-зеленый по умолчанию
                }
                
                // Если есть фактическое время взлета, но нет фактического времени посадки — используем цвет "Взлетел"
                if (currentFlight && 
                    currentFlight.actual_date_departure && currentFlight.actual_time_departure &&
                    (!currentFlight.actual_date_arrival || !currentFlight.actual_time_arrival)) {
                    const takenOffColor = resolveCrewStatusColor('Взлетел');
                    if (takenOffColor) {
                        statusColor = takenOffColor;
                    }
                }
                ...
                if(currentFlight && currentFlight.actual_time_departure && currentFlight.actual_time_arrival) {
                    ...
                    if(activity_type === 'flight') {
                        ...
                        let actualXFinishPrived = longFirstColumn + (actualXFinish/Scale);
                        
                        // Если начало прямоугольника находится раньше начала второй колонки, обрезаем его
                        if(actualXStartPrived < longFirstColumn) {
                            actualXStartPrived = longFirstColumn;
                        }
                        
                        // Вычисляем ширину прямоугольника с учетом обрезки
                        let actualXLong = actualXFinishPrived - actualXStartPrived;
                        
                        // Рисуем только если ширина больше 0
                        if(actualXLong > 0) {
                ...
                rectHover.push({
                    ...
                    actual_date_departure: currentFlight ? currentFlight.actual_date_departure : null,
                    actual_time_departure: currentFlight ? currentFlight.actual_time_departure : null,
                    actual_date_arrival: currentFlight ? currentFlight.actual_date_arrival : null,
...
function showStatusTooltip(e, crewStatus) {
    ...
    if (crewStatus.aircraft_types && crewStatus.aircraft_types.length > 0) {
        ...
        crewStatus.requirements?.forEach(req => {
                    ...
                    // Обновляем содержимое тултипа (используем отдельные ID для тултипа экипажа)
                    const status3vp = document.getElementById("status-3vp-crew");
                    const status3vpn = document.getElementById("status-3vpn-crew");
                    const statusLimits = document.getElementById("status-limits-crew");
...
function handleMouseMove(e){
    ...
    // tooltip статусов теперь управляется только кликами
    hoveredObj = "false";
...
ClickCanvas.addEventListener('click', (event) => {
   ...
   if (canvas2) {
       ...
       for(let i = 0; i < Quantity; i++) {
           ...
           if(distance <= circleRadius) {
               ...
               if (crewStatus) {
                   ...
                   return; // Выходим после обработки клика по кругу
               }
           }
       }
   }
       ...
       // Если hoveredObj уже установлен в handleMouseMove, используем его
       console.log('hoveredObj в ClickCanvas:', hoveredObj);
  ...
  // Показываем основной тултип
                ...
                // Записываем статус (используем название напрямую из БД) и красим фон строки статуса
                (function(){
...
// Обработчик двойного клика для открытия модального окна редактирования мероприятия
ClickCanvas.addEventListener('dblclick', (event) => {
    const canvas2 = document.getElementById('canvas2');
    if (!canvas2) return;
    ...
    // Ищем мероприятие в массиве rectHover
    for (let i = 0; i < rectHover.length; i++) {
        const dot = rectHover[i];
        if (!dot) continue;
        
        // Проверяем попадание в прямоугольник
        if ((mouseX > dot.x) && (mouseX < (dot.x + dot.XLong)) && 
            (mouseY > dot.y) && (mouseY < (dot.y + heightRectFlight))) {
            
            // Проверяем, что это мероприятие
            if (dot.activity_type === 'crew_events') {
                // Собираем данные о мероприятии для передачи в модальное окно
                ...
                showEditEventModal(eventDataForModal);
                return; // Выходим после открытия модального окна
            }
        }
    }
});
...
function drawPastTimeOverlay() {
    ...
    // Создаем полную дату и время для текущего момента в UTC
    const currentDateTime = new Date(todayDate);
    currentDateTime.setUTCHours(Math.floor(currentTimeOfDay / 60), currentTimeOfDay % 60);
    ...
    // Вычисляем позицию линии текущего времени
    var currentTimeX = longFirstColumn + timeDiffMinutes / Scale;
...
function showAddFlightConfirmModal(flight, crewMember) {
    ...
    modal.dataset.aircraft = flight.aircraft || '';
...
function showSelectPositionModal(flight, crewMember) {
    console.log('showSelectPositionModal called with:', { flight, crewMember });
    console.log('=== FLIGHT OBJECT STRUCTURE ===');
    console.log('Full flight object:', JSON.stringify(flight, null, 2));
    console.log('flight.aircraft:', flight.aircraft);
    console.log('flight.Aircraft:', flight.Aircraft);
    console.log('flight.RegN:', flight.RegN);
    console.log('flight.regn:', flight.regn);
    console.log('All flight keys:', Object.keys(flight));
    
    // Создаем модальное окно (используем динамическое создание, как в RectMovement.js)
    const existingModal = document.getElementById('dynamicPositionModal');
    if (existingModal) {
        existingModal.remove();
    }
    
    const modal = document.createElement('div');
    modal.id = 'dynamicPositionModal';
    modal.style.cssText = `
    ...
    modal.innerHTML = `
        <div style="background:#fff;padding:30px;border-radius:10px;min-width:320px;text-align:center">
            ...
            <p style="color:#666;margin-bottom:15px;">Сотрудник: ${crewMember.name || 'Неизвестно'}</p>
            <p style="color:#666;margin-bottom:15px;">Рейс: ${flight.flight_number || 'N/A'}</p>
            <p style="color:#666;margin-bottom:15px;">Загрузка должностей для данного типа ВС...</p>
        </div>
    `;
    document.body.appendChild(modal);
    
    // 1. Получаем регистрацию борта
    const regn = flight.aircraft || flight.Aircraft || flight.RegN || flight.regn || null;
    console.log('Detected aircraft regn for minimum crew filter:', regn, 'from flight:', flight);

    if (!regn) {
        console.error('Не удалось определить регистрацию ВС из данных рейса');
        modal.innerHTML = `
            <div style="background:#fff;padding:30px;border-radius:10px;min-width:320px;text-align:center">
                <h4>Ошибка</h4>
    ...
    fetch(`/api/aircraft-type-by-regn?regn=${encodeURIComponent(regn)}`)
        ...
        .then(minCrewData => {
            console.log('minimum-crew/by-aircraft-type response data:', minCrewData);
            ...
            // 4. Определяем тип экипажа текущего сотрудника по его должности
            return fetch('/api/positions')
                .then(resp => resp.json())
                .then(posData => {
                    let crewTypeFilter = null;
                    if (posData.success && Array.isArray(posData.data)) {
                        const crewPosCode = crewMember.position || null;
                        ...
                        if (crewPosCode) {
                            ...
                            if (matchedPosition && matchedPosition.crew_type) {
                                crewTypeFilter = matchedPosition.crew_type;
                            }
                    ...
                    groupKeys.forEach(groupKey => {
                        ...
                        groupItems.forEach((item, itemIndex) => {
                            ...
                            console.log(`Position: ${item.position_name}, quantity: ${quantity}`);
                            ...
                            if (quantity > 0) {
                                ...
                                for (let i = 1; i <= quantity; i++) {
                                    ...
                                    // Значение для отправки на сервер (всегда со слотом)
                                    const value = `${positionShortName}${i}`;
                    ...
                    // Загружаем назначенных сотрудников для отображения ФИО
                    return fetch(`/planning/flights/get-crew/${flight.id}`)
                        .then(r => r.json())
                        .then(crewResponse => {
                            let assignedCrewMap = {};
                            if (crewResponse.success && Array.isArray(crewResponse.crew_members)) {
                                crewResponse.crew_members.forEach(cm => {
                                    if (cm.position_with_slot) {
                                        assignedCrewMap[cm.position_with_slot] = cm.crew_name || cm.name;
                                    }
                                });
                            }
                            console.log('[ASSIGNED CREW] Final map:', assignedCrewMap);
                            
                            let options = '';
                            positions.forEach(position => {
                                const assignedName = assignedCrewMap[position.value];
                                const displayText = assignedName 
                                    ? `${position.text} - ${assignedName}`
                                    : position.text;
                                options += `<option value="${position.value}">${displayText}</option>`;
                            });
                            ...
                            modal.innerHTML = `
                                <div style="background:#fff;padding:30px;border-radius:10px;min-width:320px;text-align:center">
                                    <h4>Выберите позицию сотрудника</h4>
                                    <p style="color:#666;margin-bottom:10px;">Сотрудник: ${crewMember.name || 'Неизвестно'}</p>
                                    <p style="color:#666;margin-bottom:15px;font-size:13px;">
                                        Рейс: ${flight.flight_number || 'N/A'}<br>
                                        ${flight.departure_airport || ''} → ${flight.arrival_airport || ''}, ${flight.date_departure || ''} ${flight.time_departure ? flight.time_departure.substr(0,5) : ''}
                                    </p>
                                    <select id="crew-position-select-context" class="form-select mb-3" style="width:100%;padding:8px;margin-bottom:15px;">
                                        <option value="">Выберите должность</option>
                                        ${options}
                                    </select>
                                    <div style="display:flex;gap:10px;justify-content:center;">
                                        <button id="position-ok-context" style="
                                            padding: 8px 20px;
                                            background: #007bff;
                                            color: white;
                                            border: none;
                                            ...
                                            cursor: pointer;
                                        ">OK</button>
                                        <button id="position-cancel-context" style="
                                            padding: 8px 20px;
                                            background: #6c757d;
                                            color: white;
                                            border: none;
                            ...
                            document.getElementById('position-ok-context').onclick = function() {
                                const pos = document.getElementById('crew-position-select-context').value;
                                if (!pos) {
                                ...
                                assignCrewToFlight(flight, crewMember, pos);
                            };

                            document.getElementById('position-cancel-context').onclick = function() {
                                modal.remove();
                            };
...
// closeSelectPositionModal удалена - теперь используется динамическое модальное окно
...
// Старый обработчик удален - логика теперь встроена в showSelectPositionModal
...
confirmAddFlightBtn.addEventListener('click', function() {
    ...
    const flight = {
        ...
        aircraft: modal.dataset.aircraft || null,
...
function showEditEventModal(eventData) {
    ...
    const text = document.getElementById('editEventText');
    
    if (!eventData) {
        text.innerHTML = '<b>Ошибка: данные мероприятия не найдены!</b>';
        modal.style.display = 'block';
        return;
    }
    
    // Получаем данные из основного массива data
    let eventRow = null;
    try {
        if (eventData.id != null) {
            const lookup = String(eventData.id);
            eventRow = Object.values(data).find(f => f && f.activity_type === 'crew_events' && String(f.id) === lookup);
        }
    } catch (err) {
    ...
    const eventTypeName = eventData.event_type_name || eventData.flight_type || 'Мероприятие';
    const crewName = eventData.crew_name || '-';
    const crewPosition = eventData.crew_position || '';
    const eventColor = (eventRow && (eventRow.event_color || eventRow.color)) || '#007bff';
    
    text.innerHTML = `
        <b>Мероприятие: <span style="color:${eventColor}">${eventTypeName}</span></b><br>
        <span style="font-size:13px;color:#666;">Сотрудник: ${crewName}${crewPosition ? ' (' + crewPosition + ')' : ''}</span>
    `;
    
    // Заполняем поля данными из мероприятия
    ...
    // Получаем даты и время из eventRow или eventData
    const dateStart = eventRow ? (eventRow.date_departure || eventRow.date_start) : '';
    const dateEnd = eventRow ? (eventRow.date_arrival || eventRow.date_end) : '';
    const timeStart = eventData.time_start || (eventRow ? eventRow.time_start : '') || '';
    const timeEnd = eventData.time_finish || (eventRow ? eventRow.time_finish : '') || '';
    const notes = eventRow ? (eventRow.notes || eventRow.comment || eventRow.comments || '') : '';
    
    startDateInput.value = dateStart || '';
    startTimeInput.value = timeStart ? timeStart.substr(0, 5) : '';
    endDateInput.value = dateEnd || '';
    endTimeInput.value = timeEnd ? timeEnd.substr(0, 5) : '';
    notesInput.value = notes;
    
    // Сохраняем данные в dataset модалки для последующего использования
    modal.dataset.eventId = eventData.id || '';
    modal.dataset.eventTypeId = eventRow ? (eventRow.event_id || eventRow.event_type_id || '') : '';
    modal.dataset.eventTypeName = eventTypeName;
    modal.dataset.crewId = eventData.crew_id || (eventRow ? eventRow.crew_id : '') || '';
    modal.dataset.crewName = crewName;
...
// Обработчик кнопки Сохранить в модалке редактирования мероприятия
const confirmEditEventBtn = document.getElementById('confirmEditEventBtn');
if (confirmEditEventBtn) {
    confirmEditEventBtn.addEventListener('click', function() {
        saveEditEvent(false);
    });
}

function saveEditEvent(forceSave = false) {
    const modal = document.getElementById('editEventModal');
    const eventId = modal.dataset.eventId;
    const eventTypeId = modal.dataset.eventTypeId;
    const eventTypeName = modal.dataset.eventTypeName;
    const crewId = modal.dataset.crewId;
    ...
    const notes = document.getElementById('editEventNotes').value || null;
    ...
    if (forceSave) {
        requestData.force_save = true;
    }
    
    // Обновляем мероприятие через API
    fetch('/planning/crew-events/update', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    .then(data => {
        if (data.success) {
            closeEditEventModal();
                ...
                showEventNotification('Мероприятие успешно обновлено', 'success');
                // Перезагрузить данные и перерисовать canvas
                reloadCrewData().then(() => {
                    if (typeof drawTable2 === 'function') {
                        try { 
                            cleanCanvas();
                            drawTable2(); 
                            window.rectHoverCrew = rectHover;
                        } catch (e) { 
                            console.error('Ошибка перерисовки canvas:', e);
                            location.reload();
                        }
                    } else {
                        location.reload();
                ...
                // Сохраняем данные для возможности принудительного сохранения
                ...
                alert('Ошибка при обновлении мероприятия: ' + (data.message || 'Неизвестная ошибка'));
            }
        })
        .catch(err => {
...
// Обработчик кнопки Удалить в модалке редактирования мероприятия
...
if (deleteEventBtn) {
    deleteEventBtn.addEventListener('click', function() {
        ...
        if (!confirm(`Вы уверены, что хотите удалить мероприятие "${eventTypeName}" для сотрудника "${crewName}"?`)) {
            return;
        }
        
        // Удаляем мероприятие через API
        fetch('/planning/crew-events/delete', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        .then(data => {
            if (data.success) {
                closeEditEventModal();
                // Показать уведомление об успехе
                showEventNotification('Мероприятие успешно удалено', 'success');
                // Перезагрузить данные и перерисовать canvas
                reloadCrewData().then(() => {
                    if (typeof drawTable2 === 'function') {
                        try { 
                ...
                alert('Ошибка при удалении мероприятия: ' + (data.message || 'Неизвестная ошибка'));
            }
        })
        .catch(err => {
...
// Функция для показа уведомлений о мероприятиях
function showEventNotification(message, type) {
    if (typeof window.showNotification === 'function') {
        ...
        try {
            ...
            n.style.borderLeft = type === 'success' ? '4px solid #28a745' : '4px solid #dc3545';
...
// Глобальная переменная для хранения данных при конфликтах
...
confirmAddEventBtn.addEventListener('click', function() {
    saveEventAssign(false);
});

function saveEventAssign(forceSave = false) {
    const modal = document.getElementById('addEventAssignModal');
    const eventId = modal.dataset.eventId;
    ...
    .then(data => {
        if (data.success) {
            ...
            alert('Ошибка при сохранении мероприятия: ' + (data.message || 'Неизвестная ошибка'));
        }
    })
    .catch(err => {
...
// Функция для отображения модального окна с конфликтами
    ...
    conflicts.forEach(conflict => {
        ...
        const formatDate = (dateStr) => {
            ...
            try {
                ...
                const dd = (d || 0).toString().padStart(2, '0');
                return `${dd} ${mm}`;
            } catch(e) {
                return dateStr;
            }
        ...
        row.innerHTML = `
            <td>${conflict.crew_name}</td>
            <td>${conflict.type}</td>
            <td>${conflict.event_type}</td>
            <td>${formatDate(conflict.start_date)} ${formatTime(conflict.start_time)}</td>
            <td>${formatDate(conflict.end_date)} ${formatTime(conflict.end_time)}</td>
        `;
...
const forceSaveEventBtn = document.getElementById('forceSaveEventBtn');
if (forceSaveEventBtn) {
    forceSaveEventBtn.addEventListener('click', function() {
        closeConflictsModal();
        if (isEditMode) {
            saveEditEvent(true);
        } else {
            saveEventAssign(true);
        }
    });
}
...
window.showConflictsModal = showConflictsModal;